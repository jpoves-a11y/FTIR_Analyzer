...
- **Quantification**:
    - **Oxidation, Crystallinity, TVI, Vitamin E**: Indices are calculated using peak ratios with two method options:
        - **Area-based (Traditional)**: Uses integrated peak areas for ratio calculation (OI, CI Area).
        - **Height-based**: Uses maximum peak heights for ratio calculation (KOI, CI Height). Reference: Solberg P, Jevsevar K, Currier B, Orem A, Jevsevar D, Van Citters D. DOI: 10.1002/jbm.b.35627
            - KOI (Ketone Oxidation Index): max(1710-1730 cm⁻¹) / max(1360-1380 cm⁻¹)
            - CI Height: Uses max values instead of areas with same CI formula
            - Red dot markers show maximum values on interval plots when height-based method is selected
